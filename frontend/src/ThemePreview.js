...
// ===== Option 2 — "Caribbean Energetic" palette (light marketplace theme) =====
const C = {
  bg: '#FFFCF9',
  surface: '#FDF9F3',
  surfaceElevated: '#FDEEDC',
  primary: '#0369A1',
  accent: '#F97316',
  textMain: '#1E293B',
  textMuted: '#64748B',
...
const SERVICES = [
  { name: 'Food Delivery', desc: 'Order from local favourites', Icon: ShoppingBag, tint: '#F97316' },
  { name: 'Pharmacy', desc: 'Meds delivered fast', Icon: Pill, tint: '#0369A1' },
  { name: 'Groceries', desc: 'Fresh tropical produce', Icon: Package, tint: '#10B981' },
...
export default function ThemePreview() {
  return (
    <div style={{ background: C.bg, color: C.textMain, fontFamily: "'Figtree','Manrope',sans-serif", minHeight: '100vh' }} data-testid="theme-preview-option2">
      {/* Header (glassmorphism) */}
      <header
        style={{ background: 'rgba(255,252,249,0.75)', borderBottom: `1px solid ${C.border}`, backdropFilter: 'blur(16px)' }}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span style={{ fontFamily: "'Manrope',sans-serif", color: C.primary }} className="text-xl font-extrabold tracking-tight">IslandHop</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button style={{ background: C.accent, color: '#fff' }} className="text-sm font-bold px-5 py-2.5 rounded-full shadow-[0_8px_20px_rgba(249,115,22,0.25)]">Sign up</button>
        </div>
      </header>
      ...
      <section className="px-6 lg:px-12 pt-12 pb-10 grid lg:grid-cols-2 gap-10 items-center max-w-7xl mx-auto">
        <div>
          <span style={{ background: C.surfaceElevated, color: C.accent }} className="inline-block text-xs font-bold px-3 py-1.5 rounded-full mb-5">🌴 Trinidad &amp; Tobago’s Super App</span>
          <h1 style={{ fontFamily: "'Manrope',sans-serif", color: C.textMain }} className="text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-[1.05] tracking-tight">
          ...
          {/* Search bar */}
          <div style={{ background: '#fff', border: `1px solid ${C.border}` }} className="mt-7 flex items-center gap-2 rounded-full p-1.5 pl-5 shadow-[0_10px_30px_rgba(249,115,22,0.10)] max-w-lg">
      ...
      <section className="px-6 lg:px-12 py-12 max-w-7xl mx-auto">
        <h2 style={{ fontFamily: "'Manrope',sans-serif" }} className="text-2xl font-extrabold mb-1">What can we get you?</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {SERVICES.map(({ name, desc, Icon, tint }) => (
            <div key={name} style={{ background: '#fff', border: `1px solid ${C.border}` }} className="rounded-2xl p-5 transition-all hover:-translate-y-1 hover:shadow-[0_14px_34px_rgba(249,115,22,0.14)] cursor-pointer">
      ...
      <section style={{ background: C.surface }} className="px-6 lg:px-12 py-14">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <h2 style={{ fontFamily: "'Manrope',sans-serif" }} className="text-2xl font-extrabold">Popular near you</h2>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {RESTAURANTS.map(({ name, cat, rating, time, img }) => (
              <div key={name} style={{ background: '#fff', border: `1px solid ${C.border}` }} className="rounded-2xl overflow-hidden transition-all hover:-translate-y-1 hover:shadow-[0_16px_40px_rgba(15,23,42,0.12)] cursor-pointer">
      ...
      <section className="px-6 lg:px-12 py-16">
        <div style={{ background: `linear-gradient(135deg, ${C.primary} 0%, #0284C7 100%)` }} className="max-w-7xl mx-auto rounded-3xl px-8 py-12 text-center text-white relative overflow-hidden">
          <h2 style={{ fontFamily: "'Manrope',sans-serif" }} className="text-3xl font-extrabold">Earn with IslandHop</h2>
