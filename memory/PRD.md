...
**OpenAI Chat Models integration (IslandHop Assistant):** Added a customer-facing conversational AI assistant. Model **openai/gpt-5.4** via the Emergent universal key (`EMERGENT_LLM_KEY`) using `emergentintegrations` (`LlmChat`/`UserMessage`). Multi-turn memory: messages persisted in Mongo `assistant_messages` (keyed by `session_id`), recent 12 fed back as context each turn (verified: turn 2 recalled turn 1). Backend: `routers/assistant.py` (`POST /api/assistant/chat`, `GET /api/assistant/history/{session_id}`), mounted in server.py. Frontend: `AssistantWidget.js` — floating "Ask IslandHop" launcher + chat panel with typing indicator, session id in localStorage, history restore; mounted globally in App.js. System prompt is grounded (no invented vendors/prices/order-status — guides users to the right page). `emergentintegrations==0.1.0` already in requirements.txt. Verified e2e (curl multi-turn + UI screenshot). NON-streaming (send_message) for reliability in this ingress; streaming SSE is a future enhancement.
