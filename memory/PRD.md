...
- **Root cause B — `GET /orders/{id}` access denied for non-restaurant merchants:** the check only allowed `restaurant` + `driver`; **business merchants → 403**, which the tracking UI shows as "Order not found". The notification bell (`/order/<id>`) hit the same 403. **Fix:** broadened access — customer (`customer_id` or `user_id`), assigned driver, ANY vendor the user owns (restaurants/businesses/car_rental matched on restaurant_id/vendor_id/business_id), and admin/agent.
- **Merchant dashboard currency → TT$ (`VendorDashboard.js`):** added `useCurrency().format()` and replaced all hardcoded `$…toFixed(2)` USD (order card total, stats today-revenue/total-earnings, order-detail items+subtotal+delivery+total+your-payout, savings ROI banner, weekly "owed to you") with `format()` → shows TT$ by default.
- **Also:** `OrderTrackingPageWithMaps` Delivery Address now uses `formatAddress(...)` (was wrong `street_address` key rendering a bare ", ").
- **Verified (preview curl + screenshot):** customer places COD order → appears in `GET /orders` (count 1) + tracking `GET /orders/{id}` 200; business-merchant grocery order → merchant `GET /orders` count 1 + open order 200 (was 403); merchant dashboard screenshot shows "TT$2.03 / TT$17.97" (no US$). Backend + frontend compile clean.
- **⚠️ REQUIRES REDEPLOY:** user is on production — must click **Save to GitHub → Deploy** to get BOTH this session's fixes AND the prior driver-route/TT$ fixes onto the live site.
...
- **Bugs found & fixed (frontend only, DriverDashboard.js):**
  1. **Missing render:** `availableOrders` was fetched into state + refreshed on the `available_orders` WS message, but the "Available Now" pool was **never rendered in JSX**. Added the `driver-available-now-card` section (rows `available-order-<id>`, `available-accept-btn-<id>`, `available-refresh-btn`) that lists waiting pool jobs (excluding ones already directly offered) with an Accept button → `handleAcceptOrder` → `POST /orders/{id}/accept-driver`.
  2. **Runtime crash on accept:** `const { toast } = useToast()` shadowed the `sonner` `toast` import, so `toast.success()/toast.error()` in `handleAcceptOrder` were undefined → would throw. Switched to the useToast API `toast({title, description[, variant:'destructive']})` (matches the rest of the file).
- **Verified (testing_agent iter65 — 100%, backend 7/7 + frontend 2/2):** open-pool endpoint (approved driver gets pool, non-driver 404), atomic accept (first wins, 2nd → 400 "already has a driver", wrong driver_id → 403/404), new-order `available_orders` WS broadcast reachable + pool populated, bank deposit with `proof_base64` stored + shown in user & admin queues + admin approve credits wallet exactly, PayPal withdraw request created + listed. Frontend: "Available Now" card renders rows + Accept + Refresh; wallet deposit-with-proof E2E shows pending Bank deposit.
- **SAFETY (LIVE Stripe + PayPal in preview):** per user choice, PayPal payout was verified **code-path only** — NO real payout fired. Post-test cleanup: rejected the leftover $0.50 QA PayPal withdrawal (admin reject) and reset QA driver `qadrv_2978c0f6` status busy→active.
- **REQUIRES REDEPLOY (Save to GitHub → Deploy)** to reach production.
...
- **Fix (single conversion boundary — no data migration, no merchant-form changes):**
  - **Backend `create_order` (server.py ~3845):** for merchant-catalog orders (`order.items` present AND `service_type not in ('taxi','courier')`) divide `subtotal`, `delivery_fee`, `discount` and each `item.price` by `_TTD_PER_USD` (6.78) BEFORE `calculate_order_financials`, so stored order money + commission/vendor_payout/total are all USD. **Taxi & courier are exempt** (already priced in USD upstream).
  - **Frontend `CurrencyContext.js`:** new `formatTTD(ttd)` helper — displays a TT$-authored value raw in TT$ mode, `÷6.78` in US$ mode (inverse of `format()` which takes a USD base).
  - **`RestaurantMenu.js` + `MultiCart.js` (MultiCartPage only):** menu/cart prices now use `formatTTD` → show raw TT$. `MultiCheckoutPage` + `CheckoutPage` keep `format()` (orders are already USD post-conversion) → total renders via `CurrencyConverter` = TT$ primary + US$ secondary. No CheckoutPage change needed.
  - **Business sort:** `_is_test_name()` (matches test/demo/sample/dummy/example/placeholder/qa/sandbox); `/api/search`, `/api/search/featured` now rank real vendors first, test/demo last (final stable sort applied AFTER the featured interleave).
  - Fixed duplicate React key ('Popular') in RestaurantMenu category chips (deduped + keyed by index).
- **Verified:** curl — TT$45 roti order → stored subtotal 6.64 / delivery 2.21 / item 6.64 / total 11.85 (incl. flat US$3 service fee) ✅; taxi delivery_fee NOT divided ✅. testing_agent iter62: storefront raw TT$8/45/10 + US$ toggle + cart TT$45 (no inflation) + checkout dual-currency (TT$80.34 / US$11.85) = **frontend 100%**; backend 4/5 then interleave-sort bug fixed → featured now ranks 'Notify Test Diner' last. Clean compile.
- **Seeded preview test storefront:** vendor `b0ca2f1d-d696-4e77-a606-f82b44c5d817` (Roti Palace) — Doubles TT$8, Chicken Roti TT$45, Sweet Drink TT$10, delivery TT$15, min TT$20. (Preview storefront data was otherwise broken: menu_items referenced non-existent vendor docs.)
- **NOTE — flat service fee:** `PLATFORM_SERVICE_FEE` = **US$3.00 = TT$20.34**, added to every catalog order → shows large on small TT$ baskets. Env-configurable (`PLATFORM_SERVICE_FEE`); may want lowering for T&T. Out of scope this session.
- **REQUIRES REDEPLOY (Save to GitHub → Deploy)** to reach production (islandhop-mvp.emergent.host / islandhopapp.com / islandhoptt.com).
...
- **End-of-day auto settlement (merchants + drivers):** new `run_daily_settlement(actor)` engine — sums unsettled delivered-order `vendor_payout` per merchant → credits wallet (`_credit_wallet_with_txn`, USD) + marks `vendor_payout_status='settled'` + `settlements` record `external_payout_status='queued'`; sums unsettled prepaid `driver_earnings` per driver, **nets against COD `cash_outstanding`** (earnings offset owed cash, remainder paid to driver wallet), marks `driver_payout_status='settled'`, records `settlement_batches`. Idempotent. Endpoints `POST /api/admin/settlements/run` (on-demand) + `GET /api/admin/settlements`. Scheduler: replaced the old 02:00-UTC Stripe vendor-payout job with `daily_settlement` CronTrigger **04:00 UTC (= 00:00 AST end-of-day)**. Admin UI: `AdminPanel.js` orders tab → `settlement-card` + `run-settlement-btn` + recent batches. Verified end-to-end (correct netting: earnings 7 offset cash owed 10 → paid 0, remaining 3; idempotent re-run = 0).
- **Delivery Pin:** new reusable `MapPinPicker.js` (draggable Google Map pin, click-to-place, controlled lat/lng). Added to `AddressManagement.js` add/edit modal (`address-map-pin`) so customers set their exact drop-off coordinates on every saved address → flows into order `delivery_address` → drivers see precise dropoff + map link.
- **Re-seeded** `admin.qa@islandhop-demo.com` / `AdminQA1234!` (was 401 / missing) — now user_type=admin, active. Verified login 200.
- **Testing (iteration 61):** claims ✅, settlement ✅ (backend+UI+idempotency), delivery-pin UI ✅ but save was blocked by the addresses 422 — **now fixed & re-verified via curl**. All REQUIRE REDEPLOY.
...
- **Change (`server.py`, `PROMO_REWARDS`):** added `_TTD_PER_USD` (=6.78, env `RATE_TTD_PER_USD`) + `_ttd_to_usd()`; rewards now defined from TT$ targets (`PROMO_REWARD_{CUSTOMER,DRIVER,MERCHANT,SUPPLIER}_TTD` = 20/50/80/**100**) divided by the rate → stored USD, currency stays `USD` (matches the USD-based wallet). **Supplier updated to TT$100** per user follow-up.
- **Referral wallet test (end-to-end, passed):** eligible promoter (is_promoter) → referred customer registers with the promoter code (referred_by set, pending `referrals` row created) → `_maybe_complete_referral` on first paid order → promoter wallet `balances.USD` went **0 → 2.95 (= TT$20.00)**, `promo_rewards` row status **paid** amount 2.9499 USD (TT$20). Dashboard "Paid" reads `totals.paid` (format → TT$20.00).
